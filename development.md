...
Adminer, database web administration: <http://localhost:8081>

MailCatcher, email catch-all: <http://localhost:1080>
...
After you update it, run again with the production profile to include Caddy:

```bash
docker compose --profile production up --build
```

When deploying, for example in production, the main Caddy proxy is configured outside of the Docker Compose files. For local development, there's an included Caddy in `compose.override.yml` (behind the `production` profile), just to let you test that the domains work as expected, for example with `api.localhost.tiangolo.com` and `dashboard.localhost.tiangolo.com`.
...
Development URLs, for local development with Caddy reverse proxy (`--profile production`).
...
Adminer: <http://localhost.tiangolo.com:8081>
